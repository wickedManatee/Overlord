...
namespace Valve.VR.InteractionSystem
{
	...
	public class Throwable : MonoBehaviour
	{
		...
		{
			//Trigger got pressed
			if ( hand.GetStandardGripButtonDown() )
			{
				hand.AttachObject( gameObject, attachmentFlags, attachmentPoint );
				ControllerButtonHints.HideButtonHint( hand, Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger );
			}
		...
		private void OnDetachedFromHand( Hand hand )
		{
			...
			{
				velocity = Player.instance.trackingOriginTransform.TransformVector( hand.controller.velocity /15f );
				angularVelocity = Player.instance.trackingOriginTransform.TransformVector( hand.controller.angularVelocity / 15f );
				position = hand.transform.position;
			}
			//Debug.Log ("V: " + velocity + " aV: " + angularVelocity);

			Vector3 r = transform.TransformPoint( rb.centerOfMass ) - position;
			rb.velocity = velocity + Vector3.Cross( angularVelocity, r );
			rb.angularVelocity = angularVelocity;

			//Debug.Log ("RB V: " + rb.velocity + " RB aV: " + rb.angularVelocity);
